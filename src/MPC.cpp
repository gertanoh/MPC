 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) 
    ...
    // the Solver function below.
		...
		// cte, orientation, velocity to velocity ref, steering, Acceleration, change in steering, change in Acceleration
		weigths << 1, 1, 1, 5000, 50, 5000, 1;
		//weigths << 10, 10, 1, 10000, 100, 7000, 50;
    ...
    for (size_t k = 0; k < N - 1; ++k)
      ...
      AD<double> psides0 = CppAD::atan(coeffs[1] + (2 * coeffs[2] * x0) + (3 * coeffs[3]* pow(x0,2) ) );
			...
			fg[1 + psi_start + k + 1] = psi1 - (psi0 + (v0/ Lf) * delta0 * dt );			
          ...
          cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
			...
			fg[1 + epsi_start + k + 1] = epsi1 - ((psi0 - psides0) + (v0/Lf) * delta0 * dt);
		}
	}
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  std::cout << "Cost " << cost << std::endl;
	...
	{
		result.push_back(solution.x[x_start + k]);
		result.push_back(solution.x[y_start + k]);
	}
